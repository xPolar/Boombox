...
class Authorization(commands.Cog):
    ...
    @commands.command()
    async def connect(self, ctx : commands.Context, channel : Union[discord.VoiceChannel, str] = None, bot : discord.User = None, twitch : str = None) -> None:
            ...
            if not bot.bot or bot.id not in config.supported_bots:
                ...
                if not document:
                    ...
                    if twitch not in [channel["name"] for channel in document["connections"] if channel["type"] == "twitch"] and twitch not in [channel["id"] for channel in document["connections"] if channel["type"] == "twitch"]:
                        ...
                        await config.cluster.connected.documents.update_one({"channel": Int64(channel.id), "bot": Int64(bot.id)}, {"$addToSet": {"twitch": twitch}}, upsert = True)
